...
function readExcelFile(filePath) {
  ...
  const sheetName2 = workbook.SheetNames[3]; // Ganti 1 dengan nama sheet jika diketahui
...
const randomString = generateRandomString(2); // Panjang 10 karakter
...
// Fungsi login
Given('Loginnya berhasil',{ timeout: 1555000 }, async function () {
...
Given('Pergi ke menu City',{ timeout: 1555000 }, async function () {
  await this.page.getByLabel('Toggle City').click();
  await this.page.waitForSelector('div:nth-child(8)', { state: 'hidden' });
});

// Memastikan tampilan list tag
When('Saya melihat tampilan list city', async function () {
  // await page.getByRole('button', { name: 'Create New' }).waitForSelector(selector, { state: 'visible', timeout: 5000 });
});

// Klik tombol Create New
When('Klik Create New di city',{ timeout: 1555000 }, async function () {
...
// Isi data tag baru
When('Isi data nya',{ timeout: 1555000 }, async function () {
  await this.page.getByRole('textbox').first().click();
  await this.page.getByRole('textbox').first().fill(data2[1][0]);
  await this.page.getByRole('textbox').nth(1).click();
  await this.page.getByRole('textbox').nth(1).fill(randomString);

  const excelFilePath = path.resolve(__dirname, '../../../../../../Excel/B2B.xlsx');
  const filePath = excelFilePath; // Lokasi file Excel Anda
  const sheetName = "CitySave";     // Nama sheet di file Excel
