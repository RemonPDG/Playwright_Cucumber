...
// Fungsi login
Given('User mengunjungi website B2B QC - 011', { timeout: 1555000 }, async function () {
...
Given('User menginput email - 011', async function () {
  await this.page.locator('input[name="email"]').click();
  await this.page.locator('input[name="email"]').fill('~!@#$%^&*()_+|{}:"<>?');
  await this.page.locator('input[name="email"]').press('Tab');
});

Given('User menginput password - 011', async function () {
  await this.page.locator('#password').fill(data1[1][1].toString());

});

Given('User mengklik button Sign In - 011', { timeout: 1555000 }, async function () {
  try {
...
Then('User diarahkan ke dashboard - 011', async function () {
  try {
    await this.page.waitForSelector('#nb-global-spinner', { state: 'hidden' });
    await this.page.waitForSelector('div:nth-child(8)', { state: 'hidden' });
  } catch (error) {
