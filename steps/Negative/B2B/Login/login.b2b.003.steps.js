...
// Fungsi login
Given('User mengunjungi website B2B QC - 003', { timeout: 1555000 }, async function () {
...
Given('User menginput email - 003', async function () {
  await this.page.locator('input[name="email"]').click();
  await this.page.locator('input[name="email"]').fill('kurniaremon@.com');
  await this.page.locator('input[name="email"]').press('Tab');
});

Given('User menginput password - 003', async function () {
  await this.page.locator('#password').fill(data1[1][1].toString());

});

Given('User mengklik button Sign In - 003', { timeout: 1555000 }, async function () {
  try {
...
Then('User diarahkan ke dashboard - 003', async function () {
  try {
    await this.page.waitForSelector('#nb-global-spinner', { state: 'hidden' });
    await this.page.waitForSelector('div:nth-child(8)', { state: 'hidden' });
  } catch (error) {
