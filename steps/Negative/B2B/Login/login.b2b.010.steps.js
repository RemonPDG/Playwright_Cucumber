...
// Fungsi login
Given('User mengunjungi website B2B QC - 010', { timeout: 1555000 }, async function () {
...
Given('User menginput email - 010', async function () {
  await this.page.locator('input[name="email"]').click();
  await this.page.locator('input[name="email"]').fill('mega.michael71@gmail.com');
  await this.page.locator('input[name="email"]').press('Tab');
});

Given('User menginput password - 010', async function () {
  await this.page.locator('#password').fill('12345');

});

Given('User mengklik button Sign In - 010', { timeout: 1555000 }, async function () {
  try {
...
Then('User diarahkan ke dashboard - 010', async function () {
  try {
    await this.page.waitForSelector('#nb-global-spinner', { state: 'hidden' });
    await this.page.waitForSelector('div:nth-child(8)', { state: 'hidden' });
  } catch (error) {
