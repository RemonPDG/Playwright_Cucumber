...
// Fungsi login
Given('User mengunjungi website B2B QC - 007', { timeout: 1555000 }, async function () {
...
Given('User tidak menginput email - 007', async function () {
...
Given('User menginput password - 007', async function () {
  await this.page.locator('#password').fill(data1[1][1].toString());

});

Given('User mengklik button Sign In - 007', { timeout: 1555000 }, async function () {
  try {
...
Then('User diarahkan ke dashboard - 007', async function () {
  try {
    await this.page.waitForSelector('#nb-global-spinner', { state: 'hidden' });
    await this.page.waitForSelector('div:nth-child(8)', { state: 'hidden' });
  } catch (error) {
