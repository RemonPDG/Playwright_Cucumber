...
// Fungsi login
Given('User mengunjungi website B2B QC - 006', { timeout: 1555000 }, async function () {
...
Given('User menginput email - 006', async function () {
...
Given('User tidak menginput password - 006', async function () {
  await this.page.locator('#password').fill('');

});

Given('User mengklik button Sign In - 006', { timeout: 1555000 }, async function () {
  try {
...
Then('User diarahkan ke dashboard - 006', async function () {
  try {
    await this.page.waitForSelector('#nb-global-spinner', { state: 'hidden' });
    await this.page.waitForSelector('div:nth-child(8)', { state: 'hidden' });
  } catch (error) {
