...
// Fungsi login
Given('Login B2B berhasil - 002',{ timeout: 1555000 }, async function () {
...
Given('Pergi ke menu Master - 002', async function () {
  await this.page.getByLabel('Toggle Master').click();
});

Given('Pergi ke menu Core - 002', async function () {
  await this.page.getByLabel('Toggle Core').click();
});

Given('Pergi ke menu Tag - 002',{ timeout: 1555000 }, async function () {
  await this.page.getByLabel('Toggle Tag').click();
  await this.page.waitForSelector('div:nth-child(8)', { state: 'hidden' });
});

// Memastikan tampilan list tag
When('Muncul list tag - 002', async function () {
...
// Isi data tag baru
When('Mengisi semua data tanpa Mengisi Tag Name - 002',{ timeout: 1555000 }, async function () {
...
Then('Klik Save, data tidak berhasil disimpan - 002',{ timeout: 1555000 }, async function () {
 try {
